...
nsA=nsH*nn; % total Number of States (for All notes), the last state is the empty state (no sound)
...
%T_new=ASC_new(:,a0)/(eps+sum(ASC_old(:,a0))); % new estimate
...
for k2=1:size(T_old,1) % state loop for dkl
    dkl=dkl+T_old(k2).*log(T_old(k2)/T_new(k2));
end
%ig=ig+T(k0,l)*dkl;
ig=dkl;
ig=ig*1000;
...
function emd=calc_emd(ASC_new,ASC_old,a0)
