...
nbirds=2; % number of birds to simulate
...
nR=15; rs=-logspace(log10(0.1),log10(100),nR); % LO rewards
    ...
    for j=1:nR
        ...
        for i=1:N % loop over songs
            for k=1:nsS % loop over notes
                ...
                %is_lo=rand(1)<(s-1)/(ns-1); % complex model, requires increase of sensory space!!!
                
                %             NAS(s,act)=NAS(s,act)+1;
                %             nLO(s,act)=nLO(s,act)+is_lo;
        ...
        % figure(11); clf; imagesc(nLO); colormap(hot); colorbar;
        % xlabel('actions'); ylabel('states'); title('#LO(states,actions) - nLO');
        ...
        % figure(13); clf; imagesc(nLO./NAS);colormap(hot); colorbar;
        % xlabel('actions'); ylabel('states'); title('Hearing p(LO)');
        
        % figure(14); clf; imagesc(nLOD./NASD);colormap(hot); colorbar;
        % xlabel('actions'); ylabel('states'); title('Deaf p(LO)');
